...
impl BioAgentsService {
    ...
    pub async fn process_paper(&self, request: ProcessPaperRequest) -> Result<ProcessPaperResponse, AppError> {
        ...
        if !response.status().is_success() {
            ...
            return Err(AppError::ExternalServiceError(format!("BioAgents API error: {}", error_text)));
        }
        ...
        info!("Paper processing task started with ID: {}", process_response.task_id);
    ...
    pub async fn get_extracted_metadata(&self, task_id: &str) -> Result<ExtractedMetadata, AppError> {
    ...
    pub async fn search_related_entities(&self, query: &str) -> Result<Vec<BiologicalEntity>, AppError> {
    ...
    pub async fn query_agents(&self, query: &str) -> Result<(String, Vec<String>), AppError> {
        ...
        let answer = response_data["answer"].as_str()
            .ok_or_else(|| AppError::DeserializationError)?
            .to_string();
            
        let sources = response_data["sources"].as_array()
            .map(|arr| arr.iter()
                .filter_map(|v| v.as_str().map(|s| s.to_string()))
                .collect())
            .unwrap_or_else(Vec::new);
            
        info!("BioAgents query successful, answer length: {}", answer.len());
        
        Ok((answer, sources))
    }
    
    /// Add knowledge to the BioAgents system
    pub async fn add_knowledge(&self, title: &str, content: &str, keywords: &[String]) -> Result<String, AppError> {
        ...
        let id = response_data["id"].as_str()
            .ok_or_else(|| AppError::DeserializationError)?
            .to_string();
    ...
    pub async fn check_health(&self) -> Result<HealthStatus, AppError> {
        ...
        let agents_online = response_data["agents_online"].as_i64()
            .unwrap_or(0) as i32;
            
        let service_status = response_data["status"].as_str()
            .unwrap_or("unknown")
            .to_string();
        ...
        info!("BioAgents health check: {} agents online, status: {}", 
